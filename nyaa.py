...
def nyaa():
    def callback():
        ...
        try:
            ...
            messagebox.showinfo("Nyaa Scraper @eliasbenb", "Something went wrong!")
        ...
        with open(file_name,'w') as w1:
            for magnet in magnets:
                w1.write(magnet)
        messagebox.showinfo("Nyaa Scraper @eliasbenb", "Magnet links successfully exported to local directory")
        ...
        if clipboard == "Yes":
            ...
            messagebox.showinfo("Nyaa Scraper @eliasbenb", "Magnets links successfully copied to clipboard")
        else:
            pass
    ...
    def save_config():
        ...
        category = category_entry.get()
        clipboard = clipboard_combobox.get()
        with open(path+"\\nyaa_config.env", "w") as w2:
            w2.write(domain+'\n'+category+'\n'+clipboard)
    
    app = Tk()

    domain_text = StringVar(app, value="https://nyaa.si/")
    domain_label = Label(app, text="Enter Nyaa Domain Link:")
