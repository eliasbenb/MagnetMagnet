...
def tpb():
    def tpb_callback():
        ...
        try:
            ...
            messagebox.showinfo("TPB Scraper @eliasbenb", "Something went wrong!")
        ...
        with open(tpb_file_name,'w') as w1:
    ...
    def tpb_load_config():
        ...
        with open(tpb_path+"\\tpb_config.env", "r") as r1:
    ...
    def tpb_save_config():
        ...
        tpb_category = tpb_category_entry.get()
        tpb_clipboard = tpb_clipboard_combobox.get()
        with open(tpb_path+"\\tpb_config.env", "w") as w2:
            w2.write(tpb_domain+'\n'+tpb_category+'\n'+tpb_clipboard)
    
    tpb_app = Tk()

    tpb_domain_text = StringVar(tpb_app, value="https://tpb.party/")
    tpb_domain_label = Label(tpb_app, text="Enter TPB Domain Link:")
    tpb_domain_label.place(relx=(1/2), rely=(1/10), anchor="center")
    tpb_domain_entry = Entry(tpb_app, textvariable=tpb_domain_text)
    tpb_domain_entry.place(relx=(1/2), rely=(1/5), anchor="center", width=300)

    tpb_category_text = StringVar()
    tpb_category_label = Label(tpb_app, text="Enter Category String:")
    tpb_category_label.place(relx=(1/2), rely=(7/20), anchor="center")
    tpb_category_entry = Entry(tpb_app, textvariable=tpb_category_text)
    tpb_category_entry.place(relx=(1/2), rely=(9/20), anchor="center")

    tpb_clipboard_label = Label(tpb_app, text="Copy to Clipboard?")
    tpb_clipboard_label.place(relx=(1/2), rely=(3/5), anchor="center")
    tpb_clipboard_combobox = ttk.Combobox(tpb_app, values=['Yes', 'No'], state='readonly')
    tpb_clipboard_combobox.place(relx=(1/2), rely=(7/10), anchor="center")

    tpb_ok_button = Button(tpb_app, text="OK", command=tpb_callback)
    tpb_ok_button.place(relx=(1/2), rely=(9/10), anchor="center")

    tpb_load_config_button = Button(tpb_app, text ="Load Config", command=tpb_load_config)
    tpb_load_config_button.place(relx=(1/5), rely=(9/20), anchor="center")

    tpb_save_config_button = Button(tpb_app, text ="Save Config", command=tpb_save_config)
    tpb_save_config_button.place(relx=(4/5), rely=(9/20), anchor="center")
    
    tpb_app.title('TPB @eliasbenb')
    tpb_app.iconbitmap(tpb_path+'\\icon.ico')
    tpb_app.geometry('500x225')
    tpb_app.resizable(False, False)
